...
cities = ["Omaha", "Lincoln", "Kansas City", "Denver", "Chicago", "St Louis"]

genders = ["Male", "male", "M", "Female", "female", "F", " Male ", " FEMALE "]
    ...
    patients.append(
        {
            ...
            "BirthDate": f"{random.randint(1950, 2010)}-"
...
patients_df.to_csv(RAW_FOLDER / "patients_data.csv", index=False)
...
for did in range(501, 526):
    doctors.append(
        {
            "DoctorID": did,
            "DoctorName": "Dr. " + random.choice(last_names),
...
doctors_df.to_csv(RAW_FOLDER / "doctors_data.csv", index=False)
...
for aid in range(9001, 9301):
    appointments.append(
        {
            "AppointmentID": aid,
            "PatientID": random.randint(1001, 1100),
            "DoctorID": random.randint(501, 525),
            "AppointmentDate": f"2026-{random.randint(1, 12):02d}-{random.randint(1, 28):02d}",
            "BillAmount": random.choice(
                [
...
appointments_df.to_csv(RAW_FOLDER / "appointments_data.csv", index=False)
